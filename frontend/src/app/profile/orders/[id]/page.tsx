...
export default function OrderDetailsPage() {
  ...
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-6">
      <Link href="/profile" className="inline-flex w-fit items-center gap-2 text-sm font-medium text-template-pink-300 hover:text-template-pink-200">
      ...
      <section className="glass rounded-lg p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <div className="mb-2 flex items-center gap-2 text-template-pink-300">
            ...
            <p className="mt-2 text-sm text-template-white/55">
              Criado em {formatDateTime(order.created_at)}
            </p>
          </div>
          <span className="w-fit rounded-full bg-template-white/10 px-4 py-2 text-xs font-bold uppercase text-template-pink-300">
            {status}
          </span>
        </div>
      </section>
        ...
        <section className="glass rounded-lg p-6">
          ...
          <div className="flex flex-col gap-4">
            {order.items.map((item) => (
              <div key={item.id} className="flex gap-4 rounded-lg border border-template-white/10 bg-template-surface-light p-4">
                <div className="relative h-20 w-20 shrink-0 overflow-hidden rounded-lg bg-template-bg">
                  <Image
                    src={item.product_image_url || "/template-hero.jpg"}
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {item.variation && (
                    <p className="text-sm text-template-pink-300">Variação: {item.variation}</p>
                  )}
                  <p className="mt-2 text-sm text-template-white/60">
                    {item.quantity} unidade(s) x {formatMoney(item.unit_price)}
                  </p>
                </div>
                <div className="text-right text-sm font-bold text-template-white">
        ...
        <aside className="flex flex-col gap-6">
          <section className="glass rounded-lg p-6">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex justify-between text-template-white/75">
                <span>Subtotal</span>
              ...
              <div className="border-t border-template-white/10 pt-3">
              ...
              <div className="border-t border-template-white/10 pt-3 text-template-white/60">
                Entrega: {shipping}
              </div>
              {order.shipping_address_text && (
                <div className="rounded-lg border border-template-white/10 bg-template-white/5 p-3 text-template-white/65">
                  {order.shipping_address_text}
                </div>
              )}
            </div>
            ...
            {canConfirmReceived && (
              <button
                ...
                className="soft-button mt-3 w-full bg-template-pink-500 text-white hover:bg-template-pink-400"
              >
            ...
            <section className="glass rounded-lg p-6">
              ...
              <form onSubmit={handleCreateReview} className="flex flex-col gap-3">
                  ...
                  {Array.from({ length: 5 }, (_, index) => {
                    ...
                    return (
                      <button
                        ...
                        className="rounded-md p-1 text-template-pink-300 transition-colors hover:bg-template-white/10"
                        aria-label={`${value} estrelas`}
                      >
                        <Star className={`h-6 w-6 ${value <= rating ? "fill-template-pink-300" : "opacity-35"}`} />
                      </button>
                    );
                  })}
                </div>
                ...
                <input
                  ...
                  className="form-field file:mr-4 file:rounded-full file:border-0 file:bg-template-purple-600 file:px-3 file:py-1 file:text-xs file:font-semibold file:text-white hover:file:bg-template-purple-500"
                />
                <button
                  type="submit"
          ...
          <section className="glass rounded-lg p-6">
            <h2 className="mb-3 flex items-center gap-2 text-xl font-bold">
              <MessageCircle className="h-5 w-5 text-template-pink-300" />
              Suporte
            </h2>
            <p className="mb-4 text-sm leading-6 text-template-white/60">
            ...
            {order.support_request_id ? (
              <Link
                ...
                className="soft-button w-full bg-template-purple-600 text-white hover:bg-template-purple-500"
              >
                Clique aqui para suporte
              </Link>
            ) : (
              <div className="rounded-lg border border-template-white/10 bg-template-white/5 p-3 text-sm text-template-white/60">
