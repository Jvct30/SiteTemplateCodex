  ...
  return (
    <div className="max-w-5xl mx-auto w-full grid grid-cols-1 md:grid-cols-3 gap-8">
      ...
      <div className="glass h-fit rounded-lg p-6">
        <div className="w-24 h-24 bg-template-gradient rounded-full mx-auto mb-4 flex items-center justify-center text-3xl font-bold">
          {user.full_name.charAt(0)}
        </div>
        <h2 className="text-2xl font-bold text-center mb-1">{user.full_name}</h2>
        <p className="text-center text-template-white/60 mb-6">@{user.username}</p>

        <div className="space-y-4 text-sm">
          <div>
            <span className="block text-template-white/40">Email</span>
            <p>{user.email || "Não informado"}</p>
          </div>
          <div>
            <span className="block text-template-white/40">Membro desde</span>
            <p>{formatDate(user.created_at)}</p>
          </div>
        </div>

        <div className="mt-6 border-t border-template-white/10 pt-5">
          <button
            type="button"
            onClick={() => setIsAddressFormOpen((value) => !value)}
            className="soft-button w-full gap-2 bg-template-purple-600 text-white hover:bg-template-purple-500"
          >
      ...
      <div className="md:col-span-2">
        <h1 className="text-3xl font-display font-bold mb-6 flex items-center gap-3">
          <Package className="w-8 h-8 text-template-pink-400" />
          Meus Pedidos
        </h1>
        ...
        <section className="glass mb-8 rounded-lg p-6">
          <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <h2 className="flex items-center gap-2 text-xl font-bold">
              <MapPin className="h-5 w-5 text-template-pink-300" />
              Meus Endereços
            </h2>
            <button
              type="button"
              onClick={() => setIsAddressFormOpen((value) => !value)}
              className="soft-button gap-2 bg-template-surface-light text-template-white hover:bg-template-white/10"
            >
              <Plus className="h-4 w-4" />
              Criar endereço
            </button>
          ...
          {isAddressFormOpen && (
            <form onSubmit={handleCreateAddress} className="mb-5 grid gap-3 rounded-lg border border-template-white/10 bg-template-surface-light p-4 sm:grid-cols-2">
              ...
              <button type="submit" className="soft-button bg-template-pink-500 text-white hover:bg-template-pink-400">
                Salvar endereço
              </button>
            </form>
          )}
          ...
          {addresses?.length ? (
            <div className="grid gap-3">
              {addresses.map((address) => (
                <div key={address.id} className="rounded-lg border border-template-white/10 bg-template-surface-light p-4">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-template-white/60">{address.full_address}</p>
                    </div>
            ...
            <p className="rounded-lg border border-template-white/10 bg-template-white/5 p-4 text-sm text-template-white/60">
        ...
        {ordersLoading ? (
          ...
          <div className="glass rounded-lg p-8 text-center">
            <p className="text-template-white/60">Você ainda não realizou nenhum pedido.</p>
          </div>
        ) : (
          <div className="space-y-6">
            {orders?.map(order => (
              <Link
                ...
                className="glass block rounded-lg p-6 transition-colors hover:border-template-pink-300"
              >
                <div className="flex flex-wrap justify-between items-center mb-4 pb-4 border-b border-template-white/10">
                  <div>
                    <div className="font-bold">Pedido #{order.id}</div>
                    <div className="text-xs text-template-white/40">{formatDateTime(order.created_at)}</div>
                  </div>
                  <div className="flex gap-4 items-center">
                    <span className="text-xl font-bold text-transparent bg-clip-text bg-hero-gradient">
                      {formatMoney(order.total)}
                    </span>
                    <span className="rounded-full bg-template-surface-light px-3 py-1 text-xs font-bold uppercase">
                      {order.status}
                    </span>
                  </div>
                </div>
                  ...
                  {order.items.map(item => (
                    <div key={item.id} className="flex justify-between items-center text-sm">
                      <div className="flex items-center gap-2">
                        <span className="w-6 h-6 bg-template-purple-600 rounded flex items-center justify-center text-xs">
                          {item.quantity}x
                        </span>
                        <span>{item.product_name} {item.variation && <span className="text-template-pink-400">({item.variation})</span>}</span>
                      </div>
                      <span className="text-template-white/60">{formatMoney(item.unit_price)}</span>
                    </div>
                  ))}
                </div>
                <div className="mt-5 border-t border-template-white/10 pt-4 text-right text-sm font-semibold text-template-pink-300">
